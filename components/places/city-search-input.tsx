...
type Props = {
  ...
  placeholder?: string;
  /** Optional grid of popular destinations to show when nothing is selected/typed. */
  popularDestinations?: PopularPlace[];
...
const DEFAULT_POPULAR: PopularPlace[] = [
...
export function CitySearchInput({
  ...
  const popular = popularDestinations ?? DEFAULT_POPULAR;
  ...
  return (
    <div className="space-y-2">
          ...
          {searchResults.map((place) => {
            ...
            return (
              <li key={place.placeId}>
                <button
                  type="button"
                  onClick={() => resolvePlace(place.placeId, place.mainText)}
                ...
                >
                  ...
                  <span className={`shrink-0 rounded-full border px-2 py-0.5 text-[10px] font-semibold capitalize ${badgeClass}`}>
                    {place.placeType}
                  </span>
                </button>
              </li>
      ...
      {showPopularGrid && popular.length > 0 && (
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            {popular.map((dest) => (
              <button
                key={dest.placeId}
                type="button"
                onClick={() => resolvePlace(dest.placeId, dest.name)}
